...
5.extract parameters from fitm3_sim
```{r set up}
#The following functions are adapted from the hBayesDM package (Ahn et al., 2017).
# load(".../fit_m3_sim.RData")
###########################################################################individual paramters#######################################################################
# Extract from the Stan fit object
parVals <- rstan::extract(fit_m3_sim, permuted = TRUE)
...
which_indPars <- c('alpha1','alpha2','beta','tau') #vectors
...
write.csv(allIndPars,file='IndPars_fit_m3_sim.csv',row.names = FALSE)

c('alpha1','alpha2','beta','tau')


alpha1     <- parVals$alpha1             
alpha2     <- parVals$alpha2             
beta       <- parVals$beta          
tau       <- parVals$tau          


writeMat('fit_m3_sim_subwise_para.mat',alpha1=alpha1,alpha2=alpha2,beta=beta,tau=tau)
