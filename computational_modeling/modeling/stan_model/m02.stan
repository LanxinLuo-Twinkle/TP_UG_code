...
// Declare all parameters as vectors for vectorizing
  ...
  vector[Ns] alpha1_pr;  // alpha1: weights for money to dummy player
  vector[Ns] alpha2_pr;  // alpha2: weights for inequality aversion (5 - self)
  ...
  for (i in 1:Ns) {
    ...
    for (t in 1:Tsubj[i]) { //loop over the trial number of each subject. Therefore, skipping the non-responsed trials.
      ...
      // we need only compute the U of 'accpet', since when rejecting, all are 0s, and U equals 0.
       util = offer_self[i,t] + alpha1[i]*offer_tp[i,t] - alpha2[i]*fmax(5-offer_self[i,t],0);
