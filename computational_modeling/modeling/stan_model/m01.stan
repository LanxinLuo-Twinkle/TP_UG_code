...
// Declare all parameters as vectors for vectorizing
  ...
  vector[2] mu_pr; //mean of the parameters,2 paras
  vector<lower=0>[2] sigma; //variance of the parameters, 2 paras

  // Subject-level raw parameters (for Matt trick)
  vector[Ns] alpha_pr;  // alpha: weights for inequality aversion (5 - self)
  vector[Ns] tau_pr;    // tau: Inverse temperature
}

transformed parameters {
  // Transform subject-level raw parameters
  real<lower=0, upper=10> alpha[Ns];
  ...
  for (i in 1:Ns) {
    ...
    for (t in 1:Tsubj[i]) { //loop over the trial number of each subject. Therefore, skipping the non-responsed trials.
      ...
      // we need only compute the U of 'accpet', since when rejecting, all are 0s, and U equals 0.
       util = offer_self[i,t] - alpha[i]*fmax(5-offer_self[i,t],0);
